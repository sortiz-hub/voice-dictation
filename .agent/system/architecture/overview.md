...
Microphone → AudioCapture → VADFilter → StreamingProcessor → Output
               (thread)       (CPU)     ┌──────────────────┐  (stdout or
                                        │ TranscriptionEngine│  keystrokes)
                                        │ (faster-whisper)   │
                                        │ (CUDA/GPU)         │
                                        └──────────────────┘
```
...
| **VADFilter** | `vad.py` | Detects speech segments, filters silence | Silero VAD via torch.hub |
...
| **ConsoleOutput** | `output.py` | Renders confirmed/partial text to stdout | sys.stdout |
| **KeyboardOutput** | `output.py` | Types text as keystrokes into focused window | Win32 SendInput (ctypes) |
...
2. Each chunk is passed to `VADFilter.is_speech()` — Silero VAD checks 512-sample windows
...
5. After sustained silence (`min_silence_ms`, default 600ms), utterance is finalized via `processor.finish()`

### Transcription Pipeline
1. `StreamingProcessor` accumulates audio chunks in a buffer
2. Every `chunk_size` seconds (default 1.0s), full buffer is sent to `TranscriptionEngine.transcribe()`
...
### Output
- **ConsoleOutput**: Confirmed text is written to stdout. Partial text uses carriage return overwrite.
- **KeyboardOutput**: Uses Win32 `SendInput` with `KEYEVENTF_UNICODE` scan codes. Partial text is typed then erased with backspaces when updated.
...
| torch | >=2.0 | Silero VAD + CUDA runtime |
